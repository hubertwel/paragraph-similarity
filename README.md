...
Here we have the inference of paragraph vectors of each paragraph (document) together with some word wectors. I discovered a very interesting thing. Only some word vectors are computed by Doc2Vec when dbow_words was set to 1 (and of course, none word vectors when dbow_words was set to 0). Actually, only about one fifth of word vectors were computed out of almost 5,000 words in the vocabulary. I was doing an experiment trying to supercharge word vectors with tf-idf values, but they did not have any impact on Doc2Vec, so I had to withdraw from that experiment.
